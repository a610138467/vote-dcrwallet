...
// created transactions from a JSON object describing the output destinations
// and amounts.
func makeOutputs_forhex(pairs map[string]dcrutil.Amount, chainParams *chaincfg.Params) (string, error) {
       ...
       for addrStr := range pairs {
               ...
               if err != nil {
                       ...
                       return hex.EncodeToString(pkScript),err
               }
       }
       return "", nil
}
...
// It returns the transaction hash in string format upon success
// All errors are returned in dcrjson.RPCError format
func sendPairs_forhex(w *wallet.Wallet, amounts map[string]dcrutil.Amount,
       account uint32, minconf int32) (string, error) {
       outputs, err := makeOutputs_forhex(amounts, w.ChainParams())
       if err != nil {
               return "", err
       }else{
               return outputs, err
...
func sendToAddress(s *Server, icmd interface{}) (interface{}, error) {
    ...
    return sendPairs_forhex(w, pairs, udb.DefaultAccountNum, 1)
